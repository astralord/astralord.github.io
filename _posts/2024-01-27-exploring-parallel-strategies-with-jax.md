...
The described strategy above is also called **Distributed Data Parallel (DDP)** and it is different from [PyTorch definition of data paralelism](https://pytorch.org/tutorials/intermediate/ddp_tutorial.html#comparison-between-dataparallel-and-distributeddataparallel) (also check out [HuggingFace explanation](https://huggingface.co/docs/transformers/perf_train_gpu_many#data-parallelism)). [PyTorch version of DP](https://pytorch.org/docs/master/generated/torch.nn.DataParallel.html) helps to overcome slow intra-node connectivity by minimizing the amount of synchronized data and delegating a lot of data/gradient processing to one leading GPU. This, in turn, results in under-utilization of other devices. 
